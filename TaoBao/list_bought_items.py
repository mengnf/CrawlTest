...
class TaoBaoOrder(object):
    ...
    # 爬取历史订单
    def crawl_history_order(self, pageNum, newURL=None):
        order_info_list = []
    ...
    def run(self):
        ...
        while self.is_continue:
            print(f"正在抓取第{i}页...")
            order_info_list.extend(self.crawl_history_order(i))
            time.sleep(2)
            i += 1

        write_to_json(order_info_list, f'淘宝订单_{datetime.datetime.now().strftime("%Y%m%d%H%M%S")}')

        sort = ['order_id', 'order_create_datetime', 'order_amount', 'order_status', 'order_detail_url', 'title', 'skuText', 'url', 'crawl_datetime']
        # 列头命名
        columns_map = {
            'order_id': '订单编号',
